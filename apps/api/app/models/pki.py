...
class CertificateAuthority(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    parent_ca_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("certificate_authorities.id", ondelete="SET NULL"), nullable=True)
...
class Certificate(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    profile_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("certificate_profiles.id", ondelete="SET NULL"), nullable=True)
    ...
    cert_pem: Mapped[str] = mapped_column(Text, nullable=False)
    encrypted_private_key: Mapped[str | None] = mapped_column(Text, nullable=True)  # Optional (if generated server-side)
    valid_from: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)
